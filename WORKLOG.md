...
### nx2 shared — `nx-canvas-header` base component (canvas-header branch)
- **`nx2/blocks/shared/canvas-header/canvas-header.js` / `.css`**: extracted the generic workspace header (was da-live **`ew-canvas-header-base`**) so the canvas toolbar and the form workspace stop duplicating it. Bar + built-in before-panel toggle + `start` / `center` / `end` slots; emits generic **`header-toggle-before`**; exposes `part`s (`bar`, `group-*`, `toggle-before`, `btn`) so consumers style/hide the toggle (forward via `exportparts` when nested). Follows shared-component conventions (`da-lit`, `loadStyle` from `utils/utils.js`, `customElements.get` guard, light-dark CSS).
- **`nx2/img/icons/S2_Icon_SplitLeft_20_N.svg`**: nx-owned icon, loaded via `loadHrefSvg` + inlined (the `toast` pattern) rather than `${codeBase}/…#use` — cross-origin `<use href>` to the nx origin is blocked. (Open: relies on nx serving the icon with CORS, same assumption as `toast`; verify on a cross-origin `/canvas` load.)
- **Consumers (da-live)**: canvas **`ew-canvas-header`** composes the base via slots (undo/redo → start, view toggle → center, after toggle → end) and maps **`header-toggle-before`** → existing **`nx-canvas-open-panel`**; the form uses the base directly. Both import it via `getNx()`.
