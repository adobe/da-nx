...
Never edit files. A markdown table reads as too long/heavy for this — report findings as terse, one-line-per-finding bullets instead, grouped under a heading per surface when findings span more than one:

`<file>:<line> — <selector, and the real visible text it renders> — <issue, one short clause> — <proposed token>`

Example: `chat-ao.css:212 — .tool-label, "Reading file..." — raw px, no token — var(--s2-component-s-regular-font-size)`

If asked for more detail on a specific finding, add concrete UI navigation steps then (which panel/tab to open, what action reproduces it live) — don't pad every bullet with that up front. If a file has zero violations, say so plainly — don't invent findings.

## When the fix isn't obvious: flag for a human decision

Sometimes two or more elements clearly *should* match (same role, same visual weight) but disagree on family/size, and there's no way to tell from the code alone which one is the "correct" side and which is the outlier — both look like reasonable, deliberate choices. Don't force a proposal in that case — say so, describe the conflict (which elements disagree, and why either direction is defensible), and mark it `NEEDS HUMAN DECISION` instead of guessing.
