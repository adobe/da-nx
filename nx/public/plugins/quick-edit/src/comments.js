...
function markerDotPagePosition(rect, placement = 'top-left') {
  const scrollLeft = window.scrollX;
  const scrollTop = window.scrollY;
  if (placement === 'center') {
    return {
      left: rect.left + (rect.width / 2) - (MARKER_SIZE / 2) + scrollLeft,
      top: rect.top + (rect.height / 2) - (MARKER_SIZE / 2) + scrollTop,
    };
  }
  return {
...
function createMarkerDot({
  threadId, isActive, rect, overlay, ctx, placement = 'top-left',
}) {
  ...
  const { left, top } = markerDotPagePosition(rect, placement);
  // Page coordinates (see positionBox) so the marker scrolls with its content.
  dot.style.left = `${Math.max(left, 4)}px`;
  dot.style.top = `${Math.max(top, 4)}px`;
...
export function scheduleCommentMarkerLayout(ctx, root = document) {
  ...
  // Short + long delayed passes catch fonts, lazy images, and reveal animations
  // that settle without firing transitionend/animationend. Scroll/resize/RO handle
  // ongoing changes after this initial window.
