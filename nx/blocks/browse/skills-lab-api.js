...
async function loadSkillFile(skill) {
...
/**
 * List skill id → markdown body for an org/site (org fallback when site list is empty).
 * @param {string} org
 * @param {string} site
...
export async function loadSkills(org, site) {
  ...
  return skills.filter(Boolean).reduce((acc, skill) => {
...
/**
 * Save skill markdown under /.da/skills/{id}.md
 * @param {string} prefix e.g. "/org/site"
 * @param {string} id
...
export async function saveSkill(prefix, id, content) {
  ...
  const out = await daFetch(`${DA_ORIGIN}/source${path}`, opts);
  if (!out.ok) return { error: `Error saving. Status: ${out.status}` };
