...
// Preserves item identity when a reorder leaves the multiset of values unchanged
// (i.e. items were only shuffled). In that case we match by content signature
// instead of positional index, so each item keeps its previous id even though
// its pointer changes.
export function assignArrayItemIds({
  ...
  const onlyReordered = previousItems.length === nextItems.length
