...
// src/state-engine/schema.js
function unionRequired(base2 = [], next2 = []) {
  return Array.from(/* @__PURE__ */ new Set([...base2 ?? [], ...next2 ?? []]));
}
...
function resolveNode({
  ...
  if (resolved.$ref) {
    ...
    if (seenRefs.has(ref)) {
      ...
      seenRefs.delete(ref);
      resolved = mergeSchemas(derefTarget, { ...resolved, $ref: void 0 });
    }
  }
  const composition = Array.isArray(resolved.allOf) && resolved.allOf.length > 0 && { key: "allOf", entries: resolved.allOf } || Array.isArray(resolved.oneOf) && resolved.oneOf.length > 0 && { key: "oneOf", entries: resolved.oneOf } || Array.isArray(resolved.anyOf) && resolved.anyOf.length > 0 && { key: "anyOf", entries: resolved.anyOf };
  if (composition) {
    const hasDirectProperties = resolved.properties && typeof resolved.properties === "object" && !Array.isArray(resolved.properties) && Object.keys(resolved.properties).length > 0;
    ...
    resolved = { ...resolved, allOf: void 0, oneOf: void 0, anyOf: void 0 };
  }
  if (resolved.items) {
...
function getDefaults({ schema = {}, kind }) {
  return {
    ...
    minItems: kind === "array" ? schema.minItems : void 0,
    maxItems: kind === "array" ? schema.maxItems : void 0
  };
}
...
function compileNode({
  ...
  if (typeof schema?.pattern === "string" && defaults.validation.pattern !== void 0) {
    try {
  ...
  if (kind === "object") {
    ...
    const node = {
      ...base2,
      children: Object.entries(properties).map(([childKey, childSchema]) => compileNode({
        key: childKey,
        schema: childSchema ?? {},
  ...
  if (Array.isArray(schema?.enum)) {
    return { ...base2, enumValues: schema.enum };
  }
  return base2;
}
function compileSchema(rawSchema) {
...
function appendPointer({ pointer, segment }) {
  const base2 = pointer === "" || pointer === "/" ? "" : pointer.replace(/\/$/, "");
  const escaped = escapeSegment(segment);
  return base2 ? `${base2}/${escaped}` : `/${escaped}`;
}
...
function setValueAt({ data, pointer, value }) {
...
function removeValueAt({ data, pointer }) {
  ...
  const isArrayIndex = Array.isArray(parent) && Number.isInteger(index2) && index2 >= 0;
  if (isArrayIndex) {
    if (index2 >= parent.length) {
      return false;
    }
...
function clearValueAt({ data, pointer, emptyValue }) {
  const target = getParentTarget({ data, pointer });
  if (!target) {
    return false;
  }
  ...
  if (isArrayIndex) {
    ...
    parent[index2] = emptyValue;
    return true;
  }
  return removeValueAt({ data, pointer });
}
function insertValueAt({ data, pointer, value }) {
  const parentPointer = getParentPointer(pointer);
  if (!parentPointer) {
    return false;
  }
  const index2 = Number.parseInt(parsePointer(pointer).at(-1), 10);
  ...
  const insertAt = Math.max(0, Math.min(index2, ensuredArray.length));
  ensuredArray.splice(insertAt, 0, value);
  return true;
}
function definitionAt({ definition, pointer }) {
  const segments = parsePointer(pointer);
  if (!segments.length || segments[0] !== "data") {
    return null;
  }
  let node = definition;
  let index2 = 1;
  while (index2 < segments.length) {
    ...
    if (node.kind === "object") {
      ...
      node = node.item ?? null;
      if (index2 < segments.length && /^\d+$/.test(segments[index2])) {
        index2 += 1;
      }
    } else {
      return null;
...
function buildNode({
  ...
  if (definition.unsupported) {
    base2.unsupported = definition.unsupported;
  }
  if (definition.unsupportedComposition) {
    base2.unsupportedComposition = definition.unsupportedComposition;
  }
  if (Array.isArray(definition.enumValues)) {
    base2.enumValues = definition.enumValues;
  }
  let node;
  if (definition.kind === "object") {
    ...
    const items = arrValue.map((itemValue, index2) => {
      const itemPointer = appendPointer({ pointer, segment: index2 });
      return buildNode({
...
function buildModel({ definition, document }) {
  if (!definition) {
    return null;
  }
...
function nodeAt({ model, pointer }) {
  return model?.byPointer?.[pointer] ?? null;
}

// src/state-engine/validation.js
...
function pushError(errors2, instancePath, error) {
  if (errors2[instancePath] !== void 0) {
    return;
  }
  errors2[instancePath] = { instancePath, ...error };
}
function validateString({ node, errors: errors2 }) {
  const { value } = node;
  if (typeof value !== "string") {
    pushError(errors2, node.pointer, {
      keyword: "type",
      params: { type: "string" },
...
function validateNumber({ node, errors: errors2 }) {
  const { value } = node;
  ...
  const { validation = {} } = node;
  if (validation.minimum !== void 0 && value < validation.minimum) {
    pushError(errors2, node.pointer, {
      keyword: "minimum",
  ...
  if (validation.maximum !== void 0 && value > validation.maximum) {
    pushError(errors2, node.pointer, {
      keyword: "maximum",
...
function validateBoolean({ node, errors: errors2 }) {
...
function validateArray({ node, errors: errors2 }) {
  const { value } = node;
  if (!Array.isArray(value)) {
    pushError(errors2, node.pointer, {
      keyword: "type",
  ...
  if (node.minItems !== void 0 && value.length < node.minItems) {
    pushError(errors2, node.pointer, {
      keyword: "minItems",
  ...
  if (node.maxItems !== void 0 && value.length > node.maxItems) {
    pushError(errors2, node.pointer, {
      keyword: "maxItems",
...
function validateNodeValue({ node, errors: errors2 }) {
  ...
  if (node.kind === "string") {
    validateString({ node, errors: errors2 });
  } else if (node.kind === "number" || node.kind === "integer") {
    validateNumber({ node, errors: errors2 });
  } else if (node.kind === "boolean") {
    validateBoolean({ node, errors: errors2 });
  } else if (node.kind === "array") {
    validateArray({ node, errors: errors2 });
  }
}
function traverse(node, errors2) {
  if (!node) {
    return;
  }
  validateNodeValue({ node, errors: errors2 });
  emitRequiredForChildren({ node, errors: errors2 });
  if (Array.isArray(node.children)) {
    node.children.forEach((c) => traverse(c, errors2));
  }
  if (Array.isArray(node.items)) {
    node.items.forEach((c) => traverse(c, errors2));
  }
}
function validateDocument({ document, model }) {
  const errors2 = {};
  const root = model?.root;
  const data = document?.data;
  if (root && data === void 0) {
    pushError(errors2, "/data", {
      keyword: "required",
  ...
  traverse(root, errors2);
  return { errors: errors2 };
}

// src/state-engine/mutate.js
...
function setField({ document, pointer, value, node }) {
  const next2 = deepClone(document);
  const normalized = normalize(value);
  if (shouldClear(normalized)) {
    const changed = clearValueAt({ data: next2, pointer, emptyValue: fallbackEmpty(node) });
    return { document: next2, changed };
  }
  const current = valueAt({ data: next2, pointer });
  if (Object.is(current, normalized)) {
    return { document: next2, changed: false };
  }
  setValueAt({ data: next2, pointer, value: normalized });
  return { document: next2, changed: true };
}
...
function addItem({ document, pointer, itemDefinition }) {
  const next2 = deepClone(document);
  const array = ensureArray({ document: next2, pointer });
  const item = buildDefault(itemDefinition);
  const insertPointer = appendPointer({ pointer, segment: array.length });
  const changed = insertValueAt({ data: next2, pointer: insertPointer, value: item });
  return { document: next2, changed };
}
function insertItem({ document, pointer, itemDefinition }) {
  const next2 = deepClone(document);
  const item = buildDefault(itemDefinition);
  const changed = insertValueAt({ data: next2, pointer, value: item });
  return { document: next2, changed };
}
function removeItem({ document, pointer }) {
  const next2 = deepClone(document);
  const changed = removeValueAt({ data: next2, pointer });
  return { document: next2, changed };
}
function moveItem({ document, pointer, fromIndex, toIndex }) {
  const next2 = deepClone(document);
  const array = valueAt({ data: next2, pointer });
...
// src/state-engine/index.js
function parseDocument(document) {
  if (!document || typeof document !== "object" || Array.isArray(document)) {
    return null;
  }
  const next2 = deepClone(document);
  if (next2.metadata === void 0) {
    next2.metadata = {};
  }
  if (next2.metadata === null || typeof next2.metadata !== "object" || Array.isArray(next2.metadata)) {
    return null;
  }
  if (!("data" in next2)) {
    return null;
  }
  if (next2.data === null || typeof next2.data !== "object") {
    return null;
  }
  return next2;
}
function isDataEmpty(value) {
  ...
  if (Array.isArray(value)) {
    return value.length === 0 || value.every(isDataEmpty);
  }
...
function materializeDefaults(definition) {
  if (!definition || typeof definition !== "object") {
    return void 0;
  }
  if (definition.defaultValue !== void 0) {
...
function coerceData(value, definition) {
  if (!definition || value === null || value === void 0) {
    return value;
  }
  ...
  if (kind === "object") {
    ...
    for (const child of definition.children ?? []) {
      if (child.key in result) {
        result[child.key] = coerceData(result[child.key], child);
      }
    }
    return result;
  }
  if (kind === "array") {
    if (!Array.isArray(value) || !definition.item) {
      return value;
    }
...
function canAdd(definition, node) {
  ...
  return definition.maxItems === void 0 || count < definition.maxItems;
}
...
function createEngine({ schema, document, onChange } = {}) {
  ...
  function rebuildModel(nextDocument) {
    ...
    model = built;
    const { errors: errors2 } = validateDocument({ document: built.document, model });
    commitState({
      document: { values: built.document },
      model: built,
      validation: { errors: errors2 },
      schemaIssues
    });
    return true;
  }
  ...
  function setField2(pointer, value) {
    if (!canMutate()) {
      return state;
    }
  ...
  function addItem2(pointer) {
    ...
    return commit(addItem({
      document: state.document?.values,
      pointer,
      itemDefinition: def.item
    }));
  }
  function insertItem2(pointer) {
    if (!canMutate()) {
      return state;
    }
    const parentPointer = getParentPointer(pointer);
    ...
    return commit(insertItem({
      document: state.document?.values,
      pointer,
      itemDefinition: def.item
    }));
  }
  function removeItem2(pointer) {
    if (!canMutate()) {
      return state;
    }
    const parentPointer = getParentPointer(pointer);
    const { def, node } = arrayContext(parentPointer);
    if (!canRemove(def, node)) {
      return state;
    }
    return commit(removeItem({ document: state.document?.values, pointer }));
  }
  function moveItem2(pointer, fromIndex, toIndex) {
    if (!canMutate()) {
      return state;
    }
    ...
    const { def, node } = arrayContext(pointer);
    if (!canReorder(def, node)) {
      return state;
    }
    if (from >= (node.items?.length ?? 0)) {
      return state;
    }
    return commit(moveItem({
      document: state.document?.values,
      pointer,
      fromIndex: from,
      toIndex: to
...
function validateData({ schema, data } = {}) {
  ...
  const { errors: errors2 } = validateDocument({ document, model });
  const valid2 = schemaIssues.length === 0 && Object.keys(errors2).length === 0;
  return { valid: valid2, errors: errors2, schemaIssues };
}
...
function prune(value) {
  ...
  if (Array.isArray(value)) {
    const filtered = value.map(prune).filter((item) => !isEmpty2(item));
  ...
  if (typeof value === "object") {
    ...
    for (const [key, propertyValue] of Object.entries(value)) {
      ...
      if (filtered !== void 0 && !isEmpty2(filtered)) {
        result[key] = filtered;
      }
    }
...
function collectElements(node, tagName, out) {
  ...
  for (const child of node.children ?? []) {
    collectElements(child, tagName, out);
  }
}
function selectBlocks(tree) {
  const mains = [];
  collectElements(tree, "main", mains);
  return mains.flatMap((main) => directChildElements(main, "div").flatMap((row2) => directChildElements(row2, "div")));
}
...
function textContent2(node) {
  ...
  return (node.children ?? []).map(textContent2).join("");
}
...
var HTMLConverter = class {
  ...
  getProperties(block2) {
    return elementChildren(block2).reduce((rdx, row2) => {
      const cols = elementChildren(row2);
      if (cols.length < 2) {
        return rdx;
      }
      const [keyCol, valCol] = cols;
      const key = textContent2(keyCol).trim();
      if (!key) {
        return rdx;
      }
  ...
  readValue(valCol) {
    ...
    if (list) {
      return this.readListValue(list);
    }
    const text = textContent2(valCol).trim();
    if (text === "") {
      return "";
    }
  ...
  readListValue(list) {
    return elementChildren(list).filter((c) => c.tagName === "li").map((li) => {
      const text = textContent2(li).trim();
  ...
  findAndConvert(searchTerm, searchRef) {
    return this.blocks.reduce((acc, block2) => {
      const idx = searchRef ? 1 : 0;
      const matches = block2.properties.className[idx]?.toLowerCase() === searchTerm.toLowerCase();
      const isRootBlock = !searchRef && !block2.properties.className[1];
      if (matches && (searchRef || isRootBlock)) {
        const properties = this.getProperties(block2);
  ...
  getTypedValue(value) {
    ...
    const boolean2 = this.getBoolean(value);
    if (boolean2 !== null) {
      return boolean2;
    }
    ...
    const number2 = this.getNumber(value);
    if (number2 !== null) {
      return number2;
    }
    return value;
...
function convertHtmlToJson({ html: html3 } = {}) {
  if (typeof html3 !== "string" || !html3.trim()) {
    return { error: "html must be a non-empty string." };
  }
  try {
    return { json: new HTMLConverter(html3).json };
  } catch (e) {
