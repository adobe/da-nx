...
| `nx-send-prompt` | `{ text: string }` | Sets the chat input field to `text` and submits it immediately. Intended for extensions that want to trigger a fully automated prompt without user interaction. No-ops if the agent is already processing. |
...
**Extension iframe usage:** Extensions running in cross-origin iframes cannot dispatch document events directly. Use `actions.setPrompt(text)` or `actions.sendPrompt(text)` from the DA SDK — the iframe protocol relays them to `nx-set-prompt` / `nx-send-prompt` on the host document. Both actions are available on the object resolved from `DA_SDK`.
