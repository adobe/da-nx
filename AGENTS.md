...
- **Adobe Spectrum & Nexter's design language**
  ...
  - Nexter supports light & dark modes natively. All features must support light and dark mode. Helpful tip: vars & `light-dark()` in CSS will do the bulk of the heavy lifting.
...
- "Parse, don't validate" — Push decisions to the boundary where data enters the system. A parser converts raw input into a structured result that downstream code can trust without re-checking. If the input is invalid, return `null` (or an error shape) rather than an object full of empty strings or defaults that every consumer must second-guess. This means:
  - Functions that interpret raw input (URL hashes, API responses, user input) should return either a well-formed result or `null` — no ambiguous middle ground.
  - Once data has been parsed, consuming code should be able to use it directly. If you find yourself re-validating fields that a prior function already checked, the parser isn't doing its job.
  - Prefer distinct values over overloaded ones: `null` means absent, `undefined` means not yet loaded, an empty string means the user explicitly cleared a field. Each carries different meaning and should not be used interchangeably.
  - This pairs with the `undefined` vs empty array convention above — both are about making the data shape itself communicate state, so branching logic stays at the edges and inner code stays clean.
