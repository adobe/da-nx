/* @ts-self-types="./fig_inspector.d.ts" */
 ...
 * @returns {Uint8Array}
 */
export function get_entry(bytes, name) {
    const ptr0 = passArray8ToWasm0(bytes, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ptr1 = passStringToWasm0(name, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len1 = WASM_VECTOR_LEN;
    const ret = wasm.get_entry(ptr0, len0, ptr1, len1);
    if (ret[3]) {
 ...
 * @returns {string}
 */
export function inspect_fig(bytes) {
    let deferred3_0;
    let deferred3_1;
    try {
        const ptr0 = passArray8ToWasm0(bytes, wasm.__wbindgen_malloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.inspect_fig(ptr0, len0);
        var ptr2 = ret[0];
        var len2 = ret[1];
        if (ret[3]) {
            ptr2 = 0; len2 = 0;
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('fig_inspector_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
