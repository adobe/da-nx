...
async function deleteDaSnapshotDirectory(name) {
  ...
  const resp = await daFetch({ url: `${DA_ORIGIN}/source/${org}/${site}/.snapshots/${name}`, opts });
  if (!resp.ok) return formatError(resp);
  return { success: true };
}
...
async function directDaCopy(url) {
  const srcResp = await daFetch({ url: `${DA_ORIGIN}/source${url.source}` });
  if (!srcResp.ok) {
    url.status = 'error';
    return;
  }
...
export async function copyManifest(name, resources, direction, mode = 'merge') {
  ...
  const { mergeCopy, overwriteCopy } = await import(`${nxBase}/blocks/loc/project/index.js`);
