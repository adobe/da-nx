...
import { daFetch, ping, source, list, signout } from '../../../../utils/api.js';
...
describe('api', () => {
  ...
  describe('ping', () => {
    it('calls HLX_ADMIN/ping with org and site', async () => {
      const headers = new Map([
        ['link', '<url>; rel="edit"'],
        ['x-api-upgrade-available', 'true'],
      ]);
      fetchStub.resolves({ headers: { get: (k) => headers.get(k) } });

      const result = await ping({ pathDetails: { org: 'myorg', site: 'mysite' } });
      const [url] = fetchStub.firstCall.args;
      expect(url).to.equal(`${HLX_ADMIN}/ping/myorg/mysite`);
      expect(result.link).to.equal('<url>; rel="edit"');
      expect(result.upgrade).to.equal('true');
    });
  ...
  describe('source', () => {
    it('fetches html source for non-sheet views', async () => {
      fetchStub.resolves(mockResp({ content: 'hello' }));
      await source({
        pathDetails: { view: 'edit', org: 'o', site: 's', path: 'folder/page' },
      });
      const [url, opts] = fetchStub.firstCall.args;
      expect(url).to.include('/o/sites/s/source/folder/pagehtml');
      expect(opts.headers['Content-Type']).to.equal('text/html');
    });

    it('fetches json source for sheet view', async () => {
      fetchStub.resolves(mockResp({ data: [] }));
      await source({
        pathDetails: { view: 'sheet', org: 'o', site: 's', path: 'data/sheet' },
      });
      const [url, opts] = fetchStub.firstCall.args;
      expect(url).to.match(/json$/);
    ...
    it('passes body for write operations', async () => {
      fetchStub.resolves(mockResp());
      await source({
        pathDetails: { view: 'edit', org: 'o', site: 's', path: 'page' },
        body: '<h1>hi</h1>',
        method: 'PUT',
      });
      const [, opts] = fetchStub.firstCall.args;
      expect(opts.method).to.equal('PUT');
      expect(opts.body).to.equal('<h1>hi</h1>');
    });
  ...
  describe('list', () => {
    it('includes path in URL when provided', async () => {
      fetchStub.resolves(mockResp([]));
      await list({ pathDetails: { org: 'o', site: 's', path: 'folder' } });
      const [url] = fetchStub.firstCall.args;
      expect(url).to.include('/o/sites/s/source/folder/');
    });

    it('uses root prefix when no path', async () => {
      fetchStub.resolves(mockResp([]));
      await list({ pathDetails: { org: 'o', site: 's' } });
      const [url] = fetchStub.firstCall.args;
      expect(url).to.include('/o/sites/s/source/');
    });

    it('returns null on failure', async () => {
      fetchStub.rejects(new Error('fail'));
      const result = await list({ pathDetails: { org: 'o', site: 's', path: 'f' } });
      expect(result).to.be.null;
    });
  });
